...
class ChangePaymentRequisites(StatesGroup):
    requisites = State()
    confirm = State()


@router.callback_query(F.data.startswith("change_payment_requisites_"))
async def change_payment_requisites(
    callback: CallbackQuery, state: FSMContext, bot: Bot
):
    project_id = callback.data.split("change_payment_requisites_")[-1]
    await state.update_data({"project_id": project_id})
    await state.set_state(ChangePaymentRequisites.requisites)
...
@router.message(ChangePaymentRequisites.requisites)
async def change_payment_requisites_process(
    ...
    kb = [
        [
            InlineKeyboardButton(
                text="✅ Да",
                callback_data=f"confirm_change_requisites_payment_{project_id}",
            ),
        ],
        [
            InlineKeyboardButton(
...
@router.callback_query(F.data.startswith("confirm_change_requisites_payment_"))
async def confirm_change_payment_requisites(
    callback: CallbackQuery, state: FSMContext, bot: Bot
):
    data = await state.get_data()
    project_id = data.get("project_id")
